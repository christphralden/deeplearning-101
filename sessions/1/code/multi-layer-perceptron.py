...
def normalize_feature_data(
    ...
    if debug:
        ...
        feature_names = scaler.get_feature_names_out(input_features=features.columns)
...
def main():
    ...
    features = normalize_feature_data(
        features=features, scaler=StandardScaler(), debug=True
    )

    x_train, x_val, x_test, y_train, y_val, y_test = train_test_val_split(
        data=(features, label), debug=True
    )
